...
{
    // SW-less coverage for the legacy-config -> v2 (World + TopLevelBodies)
    // migration paths. These guard the contract that older configurations
    ...
    // sites empty.
    public class TestLegacyToV2Migration
    {
        [Fact]
        ...
        [Fact]
        public void TestLegacyMigrationRoundTripThroughConfigV2()
        {
            // After migration, saving + reloading via ConfigV2 must
            ...
            ConfigV2 saved = ConfigV2Bridge.CreateFromLinkNode(migrated, "robot");
            string json = ConfigV2JsonSerializer.Serialize(saved);
            ConfigV2 reloaded = ConfigV2JsonSerializer.Deserialize(json);
