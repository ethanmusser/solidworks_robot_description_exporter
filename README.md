...
1. Click the `URDF/MJCF Exporter` command on the SW2RD ribbon tab.
1. Configure your export as described below.
1. Click `Export`, choose the output file, and review any validation messages shown before export completes.

### Tool Overview

The UI consists of a common link tree and a series of tabs for configuring the actively-selected link.
...
The following tabs configure the links in your tree and the way they interact with one another:

- `Setup` - Global configuration for the export, including output format, mesh format, and whether mesh files should be regenerated.  This tab also includes model-level actions for clearing saved `SW2RD` configuration data or importing legacy configuration data from the upstream `SW2URDF` tool.
...
- `Collision` - Per-link specification of assembly components that should be exported and used for collision in a simulation environment.  Collision groups can be configured independently or made identical to the `Visual` tab.
...
- `Sites` - Per-link specification of sites, which are named coordinate frames that are explicitly included in MJCF exports.  Sites are not present in URDF exports.
